...
{  
    ...
    if(gtid < num_prims)
    {
        ...
        outverts[outv0Idx].position = mul(mul(float4(in_vertices[inputvert_start].position, 1), in_instances[payload.instance_indices[gid]].mat), Globals.viewproj);
        outverts[outv1Idx].position = mul(mul(float4(in_vertices[inputvert_start + 1].position, 1), in_instances[payload.instance_indices[gid]].mat), Globals.viewproj);
    }
}